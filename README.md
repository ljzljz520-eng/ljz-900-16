...
- **管理员-检查上传**：登录后打开 `/admin`，选择员工、一次选择多张问题图片，逐张补检查项与扣分值（选检查项自动带出标准扣分，可手动修改）后一起保存；保存按图片逐张入库，某张缺信息或保存失败会标红并指出是第几张，已保存的图片不会因后面失败而丢失（失败的保留可补全后重试）；每张显示 key #1、#2… 与检查项、扣分值，可删除单张（删除后序号自动连续），保存后获得整改链接与二维码。
